...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // make sure you comment it out when you add your own code -- otherwise e.g. you might integrate yaw twice
    ...
    // Nonlinear Complementary Filter, Quan[12] Section 9.1.3 using quaternions
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    // Rotate acceleration from body to global/inertial frame
    V3F ret = attitude.Rotate_BtoI(accel);

    // Transition function = g(x_t, u_t, delta_t), state prediction
    predictedState(0) += curState(3) * dt;
    predictedState(1) += curState(4) * dt;
    predictedState(2) += curState(5) * dt;
    predictedState(3) += ret.x * dt;
    predictedState(4) += ret.y * dt;
    predictedState(5) += -9.81f * dt + ret.z * dt;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  gPrime.setIdentity();
    ...
    ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    MatrixXf pCov(QUAD_EKF_NUM_STATES, QUAD_EKF_NUM_STATES);
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    // Current estimated state vector given by the observation function
    zFromX(0) = ekfState(0);
    zFromX(1) = ekfState(1);
    zFromX(2) = ekfState(2);
    zFromX(3) = ekfState(3);
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    hPrime(0,6) = 1;
    // Current estimated Yaw given by the observation function h(xt)
    zFromX(0) = ekfState(6);
    // normalize yaw to -pi .. pi
